...
Watering_Calendar = 1 # 1 for ON, 0 for OFF
...
def get_Xmas_Lights_Month():
    global CurrentMonth
    if  CurrentMonth == 12:
        activate_xmas_lights = 1
    else:
...
# Channel names (Switch to, change to, show, TV to, whats on, watch)
    ...
    if "channels" in phrase: 
        ...
        if "gb news" in phrase or "g b news" in phrase:
            os.system ("sky-remote-cli " + IP + " " + SkyChannelList.gbnews)
            log_channel(phrase,IP,SkyChannelList.gbnews)

        if "euro news" in phrase:
            os.system ("sky-remote-cli " + IP + " " + SkyChannelList.euronews)
            log_channel(phrase,IP,SkyChannelList.euronews)
...
# Search for programmes
    if "search" in phrase or "look" in phrase:
        my_str = phrase
